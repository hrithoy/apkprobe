...
BANNER_TEXT = """
  \033[1;92mAPKprobe v0.4.0 - \033[3mScan for secrets, endpoints, and other sensitive data
  after decompiling and deobfuscating Android files.\033[0m\033[0;92m
  (.apk, .xapk, .dex, .jar, .class, .smali, .zip, .aar, .arsc, .aab, .jadx.kts)

  \033[0m\033[2m(c) wadingporque, 2025. License information in LICENSE file.
...
def main():
    ...
    if not args.quiet:
        ...
        print('\033[1mStarting APKprobe...\033[0m\n')
    ...
    try:
        apk_probe.decompile_and_scan(args.files)
    except KeyboardInterrupt:
        print("\n\033[1;34mWriting output, cleaning up, and exiting...\033[0m")
    finally:
        apk_probe.write_output()
        apk_probe.do_cleanup()

    if apk_probe.secrets_results:
        print(f"\033[1;32m\nAPKprobe done. Secrets saved to {apk_probe.output_file}\033[0m")
        exit(0)
    else:
        print("\033[1;32m\nAPKprobe done. \033[1;31mNo secrets found.\033[0m")
        exit(1)
